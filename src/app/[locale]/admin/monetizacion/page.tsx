...
export default async function AdminMonetizacionPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    <div className="flex-grow bg-slate-50/50 dark:bg-slate-900/10 py-10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-6">
          ...
          <span>/</span>
          <Link href={`/${locale}/admin`} className="hover:text-stone-700 dark:hover:text-white">
            {locale === 'en' ? 'Administration' : 'Administración'}
          </Link>
          <span>/</span>
          <span className="text-slate-600 dark:text-slate-350 font-semibold font-sans">
            {locale === 'en' ? 'Monetization & SINPE' : 'Monetización & SINPE'}
          </span>
        </div>
        ...
        <div className="mb-10 text-center sm:text-left">
          <h1 className="font-display text-3xl font-extrabold text-slate-900 dark:text-white">
            {locale === 'en' ? 'Payment Approval Cabin (SINPE Móvil)' : 'Cabina de Aprobación de Pagos (SINPE Móvil)'}
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            {locale === 'en'
              ? 'Audit manual bank transfers, view receipts, and activate premium plans or highlights in one click.'
