...
export async function generateMetadata({ params }: ComprarPageProps): Promise<Metadata> {
  ...
  const isEn = lang === 'en';
  const title = isEn ? 'Buy Properties in Costa Rica | TicoHabitat' : 'Comprar Propiedades en Costa Rica | TicoHabitat';
  const description = isEn 
    ? 'Find houses, apartments, lots and country estates for sale in Costa Rica. Direct deal with owner, cellphone OTP verification, commission-free.'
    : 'Encuentre casas, apartamentos, lotes y quintas en venta en Costa Rica. Trato directo con el propietario, verificación OTP celular y libre de intermediarios.';

  return {
    title,
    description,
    alternates: {
      canonical: `/${lang}/comprar`,
    },
    openGraph: {
      title,
      description,
      url: `https://ticohabitat.com/${lang}/comprar`,
...
export default async function ComprarIndex({ params }: ComprarPageProps) {
  const { locale } = await params;
  const lang = locale === 'en' || locale === 'es' ? locale : 'es';
