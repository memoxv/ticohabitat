...
export async function generateMetadata({ params }: AlquilarPageProps): Promise<Metadata> {
  const { locale } = await params;
  const lang = locale === 'en' || locale === 'es' ? locale : 'es';
  const isEn = lang === 'en';
  const title = isEn ? 'Rent Properties in Costa Rica | TicoHabitat' : 'Alquiler de Propiedades en Costa Rica | TicoHabitat';
  const description = isEn 
    ? 'Find houses, apartments, spaces and commercial properties for rent in Costa Rica. Direct contact, SMS/OTP cell verification.'
    : 'Descubra casas, apartamentos, cuartos y locales en alquiler en Costa Rica. Trato directo, verificación OTP contra spam y entorno 100% confiable.';

  return {
    title,
    description,
    alternates: {
      canonical: `/${lang}/alquilar`,
    },
    openGraph: {
      title,
      description,
      url: `https://ticohabitat.com/${lang}/alquilar`,
...
export default async function AlquilarIndex({ params }: AlquilarPageProps) {
  const { locale } = await params;
  const lang = locale === 'en' || locale === 'es' ? locale : 'es';
