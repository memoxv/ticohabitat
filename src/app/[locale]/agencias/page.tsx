...
export async function generateMetadata({ params }: AgenciasPageProps): Promise<Metadata> {
  const { locale } = await params;
  const lang = locale === 'en' || locale === 'es' ? locale : 'es';
  const isEn = lang === 'en';
  return {
    title: isEn ? 'Real Estate Agencies Directory | TicoHabitat' : 'Directorio de Inmobiliarias y Agencias | TicoHabitat',
    description: isEn 
      ? 'Find the most prominent professional real estate agencies in Costa Rica. Verified brokers with direct deals and listings.'
      : 'Encuentre las inmobiliarias y agencias profesionales más destacadas en Costa Rica. Asesores verificados con trato directo y listings premium.',
    alternates: {
      canonical: `/${lang}/agencias`,
    },
  };
}

export default async function AgenciasPage({ params, searchParams }: AgenciasPageProps) {
  const { locale } = await params;
  const lang = locale === 'en' || locale === 'es' ? locale : 'es';
    ...
    <div className="flex-grow bg-stone-50/20 dark:bg-stone-950/20 py-16">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-stone-400 dark:text-stone-555 mb-8 uppercase tracking-wider">
          <Link href={`/${lang}`} className="hover:text-primary transition-colors">{t.common.home}</Link>
          <span>/</span>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <form method="GET" action={`/${lang}/agencias`} className="w-full md:w-80 flex gap-2">
            ...
            {enrichedAgencies.map((agency) => (
              ...
              >
                ...
                <div className="space-y-4 pt-4 border-t border-stone-100 dark:border-stone-850/60">
                  ...
                  <Link
                    href={`/${lang}/comprar?search=${encodeURIComponent(agency.agencyName || '')}`}
