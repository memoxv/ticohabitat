...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const url = `https://ticohabitat.com/${resolvedParams.locale}/propiedad/${resolvedParams.slug}`;

  return {
    title: seoTitle,
    description: seoDescription,
    alternates: {
      canonical: `/${resolvedParams.locale}/propiedad/${resolvedParams.slug}`,
    },
    openGraph: {
      title: seoTitle,
      description: seoDescription,
...
export default async function PropertyDetailsPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  if (!property) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-12 text-center bg-stone-50/20 dark:bg-stone-950/20">
        ...
        <Link href={`/${locale}`} className="bg-stone-950 hover:bg-stone-850 dark:bg-stone-100 dark:hover:bg-stone-200 text-white dark:text-stone-900 font-bold px-6 py-3 rounded-lg shadow-sm transition-colors">
          Volver al Inicio
        </Link>
      </div>
    );
  ...
  const schemaJson = {
    ...
    url: `https://ticohabitat.com/${locale}/propiedad/${property.slug}`,
    ...
    <div className="flex-grow bg-stone-50/10 dark:bg-stone-950/10 pb-20">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <Link
            href={`/${locale}/${property.type === 'buy' ? 'comprar' : 'alquilar'}/${property.province.toLowerCase().replace(/ /g, '-')}`}
          ...
          <div className="flex items-center gap-2 text-[10px] text-stone-400 font-bold uppercase tracking-wider">
            ...
            <span>/</span>
            <Link href={`/${locale}/${property.type === 'buy' ? 'comprar' : 'alquilar'}`}>{typeLabel}</Link>
            <span>/</span>
