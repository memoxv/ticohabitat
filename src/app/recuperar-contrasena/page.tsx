...
export default function RecuperarContrasenaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !email.includes('@')) {
      showToast('Introduce una dirección de correo electrónico válida.', 'error');
      return;
    }
        ...
        <div className="bg-white dark:bg-stone-900 border border-stone-250/70 dark:border-stone-800 rounded-2xl p-8 sm:p-10 shadow-sm space-y-6">
          ...
          {!submitted ? (
            <>
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-stone-500 dark:text-stone-400 font-semibold leading-relaxed">
                  Ingresá tu correo electrónico de registro y te enviaremos un enlace seguro para restablecerla.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="input-premium py-2.5 pl-10 text-xs"
                    />
            ...
            <div className="text-center space-y-6 animate-fadeIn py-2">
              ...
              {demoLink && (
                <div className="border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-950/60 rounded-xl p-4 text-left space-y-2">
                  <div className="flex items-center gap-1.5 text-[9px] font-bold text-amber-500 tracking-wider uppercase">
                    <Sparkles className="h-3.5 w-3.5 text-amber-500 animate-pulse" />
                    <span>Enlace Demo Beta (Copia y pega):</span>
                  </div>
                  <div className="text-[10px] font-mono select-all break-all text-stone-300">
                    {demoLink}
                  </div>
                </div>
              )}
