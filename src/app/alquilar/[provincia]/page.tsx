...
export async function generateMetadata({ params }: { params: Promise<{ provincia: string }> }) {
  ...
  return {
    ...
    alternates: {
      canonical: `/alquilar/${resolvedParams.provincia}`,
    },
    openGraph: {
      title: `Propiedades en Alquiler en ${province} | TicoHabitat`,
      description: `Encuentre casas, apartamentos y locales en alquiler en ${province}, Costa Rica. Filtrado rápido, libre de spam y verificado por OTP.`,
  ...
  if (!provinceName) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-12 text-center">
        ...
        <Link href="/alquilar" className="bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-xl text-sm font-semibold shadow">
          Volver a Selección
        </Link>
      </div>
    );
    ...
    <div className="flex-grow bg-slate-50/50 dark:bg-slate-900/10">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
          ...
          <span>/</span>
          <Link href="/alquilar" className="hover:text-primary transition-colors">Alquilar</Link>
          <span>/</span>
