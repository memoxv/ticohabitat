...
export default function RegistroPage() {
  ...
  const { login, showToast, verifyPhoneInSession } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sessionUser) {
      showToast('Cuenta creada. Enviamos un enlace de validación a tu correo.', 'success');
        ...
        <div className="bg-white dark:bg-stone-900 border border-stone-250/70 dark:border-stone-800 rounded-2xl p-8 sm:p-10 shadow-sm space-y-6">
          <div className="text-center space-y-2">
            <h2 className="font-display text-xl font-extrabold text-stone-900 dark:text-white tracking-tight">
              Creá tu cuenta de anunciante
            </h2>
            <p className="text-xs text-stone-450 dark:text-stone-500 font-semibold leading-relaxed">
              Unite a la comunidad inmobiliaria más confiable del país.
            </p>
          </div>
          ...
          {phoneInUseError && (
            <div className="border border-red-500/20 bg-red-500/5 text-stone-800 dark:text-stone-200 rounded-xl p-5 text-xs space-y-3.5 animate-fadeIn">
              <div className="flex items-center gap-2 font-bold text-red-650 dark:text-red-400 tracking-wide uppercase text-[10px]">
                ...
                <span>Número telefónico ya registrado</span>
              </div>
              ...
              <div className="flex flex-col gap-2 pt-1">
                ...
                <a
                  href={`https://wa.me/50660677055?text=Hola!%20Estoy%20intentando%20registrarme%20en%20TicoHabitat%20con%20el%20número%20${duplicatePhone}%20pero%20indica%20que%20ya%20está%2520en%2520uso.%20¿Me%20podrían%20ayudar?%20Mi%20nombre%2520es%2520${encodeURIComponent(name || '[Mi Nombre]')}.`}
                  target="_blank"
                  rel="noreferrer"
                ...
                >
                  ...
                  <span>Enviar WhatsApp a Soporte</span>
                </a>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wider mb-2">Nombre Completo</label>
              <div className="relative">
                <input
                  type="text"
                  placeholder="Carlos Vargas"
                  value={name}
            ...
            <div>
              <label className="block text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wider mb-2">Contraseña</label>
              <div className="relative">
                <input
                  type="password"
                  placeholder="••••••••"
          ...
          <p className="text-center text-xs text-stone-400 dark:text-stone-500 mt-6 font-medium">
            ¿Ya tiene una cuenta? <Link href="/login" className="text-primary hover:underline font-bold">Inicie sesión aquí</Link>
          </p>
