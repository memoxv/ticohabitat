...
export default async function ComprarIndex() {
  ...
  let featuredTotal = 0;
  try {
    const result = await getRandomFeaturedPropertiesAction('buy', 3);
    featuredItems = result.items;
    featuredTotal = result.totalCount;
  } catch (err) {
    console.error('Failed to fetch buying featured properties, falling back to empty list:', err);
  }

  // Query real active listing counts per province, filtered by type='buy'
    ...
    <div className="flex-grow bg-stone-50/20 dark:bg-stone-950/20 py-16">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-stone-400 dark:text-stone-500 mb-8">
          ...
          <span>/</span>
          <span className="text-stone-600 dark:text-stone-300">Comprar</span>
        </div>
        ...
        <div className="text-left mb-16 max-w-2xl">
          <h1 className="font-display text-4xl sm:text-5xl font-extrabold text-stone-900 dark:text-white tracking-tight leading-tight">
            ¿En cuál provincia desea <span className="text-primary">adquirir</span> su propiedad?
          </h1>
          <p className="text-sm font-medium text-stone-500 dark:text-stone-400 mt-4 leading-relaxed">
            Explore los anuncios activos de venta en Costa Rica. Cada publicación cuenta con filtros de calidad, verificación obligatoria contra spam y enlace directo al propietario.
          </p>
        </div>
        ...
        {featuredItems.length > 0 ? (
          <div className="mb-16">
            <FeaturedCarousel
              ...
              typeFilter="buy"
              title="Recomendados Premium en Venta"
          ...
          <div className="mb-16 p-8 rounded-3xl border border-amber-500/20 bg-gradient-to-r from-amber-500/5 via-amber-500/0 to-transparent flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              ...
              <h3 className="font-display font-extrabold text-lg text-stone-900 dark:text-white">
                ¿Tenés una propiedad en venta? Destacala aquí
              </h3>
              <p className="text-xs text-stone-500 dark:text-stone-400 max-w-xl leading-relaxed">
                Hacé que tu anuncio sea lo primero que vean miles de compradores costarricenses al ingresar a TicoHabitat. Aumentá el interés en tu propiedad al instante.
              </p>
            </div>
            <Link 
              href="/publicar" 
