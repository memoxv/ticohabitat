...
export default function AdminMonetizacionClient({ initialTransactions }: AdminMonetizacionClientProps) {
  ...
  const handleAction = async (txId: string, status: 'approved' | 'rejected') => {
    const notes = rejectNotes[txId] || (status === 'approved' ? 'Comprobante SINPE validado con éxito. ¡Destaque/Plan Activo!' : 'Transacción rechazada.');
    
    if (status === 'rejected' && !rejectNotes[txId]?.trim()) {
      showToast('Por favor escribe una nota explicando la razón del rechazo.', 'error');
      return;
    }
    ...
    if (res.success) {
      ...
      showToast(res.message || 'Error al procesar moderación.', 'error');
    }
  };
          ...
          {transactions.map((tx) => {
            ...
            return (
              ...
              >
                ...
                <div className="p-6 lg:p-8 lg:col-span-2 space-y-6 flex flex-col justify-between">
                  <div className="space-y-5">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div>
                        {isFeaturedTx && (
                          <span className="inline-flex items-center gap-1 bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border border-amber-250/25">
                            <Sparkles className="h-3.5 w-3.5 text-amber-500 shrink-0" />
                            <span>Destaque Individual ({tx.durationDays} Días)</span>
                          </span>
                        )}

                        {isPremiumTx && (
                          <span className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-500/10 to-yellow-500/10 text-amber-700 dark:text-amber-450 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border border-amber-300/30">
                            <Sparkles className="h-3.5 w-3.5 text-amber-500 shrink-0 animate-pulse" />
                            <span>Plan Corredor Premium ⭐</span>
                          </span>
                        )}

                        {isAgencyTx && (
                          <span className="inline-flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-850 dark:text-emerald-400 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border border-emerald-250/25">
                            <Building className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                            <span>Plan Inmobiliaria / Agencia 🏢</span>
                          </span>
                        )}
                      </div>

                      <div className="font-display font-black text-2xl text-emerald-700 dark:text-emerald-450">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 bg-background p-5 rounded-xl border border-card-border">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-xs font-bold text-stone-850 dark:text-stone-100">
                          {tx.user.name || 'Usuario TicoHabitat'}
                        </div>
                              ...
                              return (
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-extrabold text-stone-800 dark:text-white uppercase tracking-wide">
                                    🏢 {parsed.agencyName || 'Sin nombre'}
                                  </span>
                                </div>
                              );
                            } catch {
                              return <span className="text-stone-500">Formato de datos de inmobiliaria corrupto.</span>;
                            }
                          })()}
                        </div>
                      )}
                  ...
                  <div className="space-y-4 pt-4 border-t border-stone-100 dark:border-stone-850">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold uppercase text-stone-400">
                        Notas de Moderación / Feedback al Anunciante (Obligatorio en Rechazos)
                      </label>
                      <input
                        type="text"
                        placeholder="Ej. Comprobante validado con éxito. / Referencia bancaria no encontrada, favor re-enviar comprobante."
                ...
                >
                  <div className="absolute inset-0 bg-stone-950/40 opacity-0 group-hover/receipt:opacity-100 transition-opacity flex items-center justify-center z-10">
                  ...
                  <span className="text-[10px] uppercase font-bold text-stone-400 mt-4 flex items-center gap-1">
                    ...
                    Haga clic para ampliar captura
                  </span>
                </div>
              </div>
            );
